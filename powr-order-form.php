<?php
    /**
     * @package POWr Order Form
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Order Form
    Plugin URI: https://www.powr.io/plugins/order-form
    Description: Sell anything on any site.  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Order_Form extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_order_form',
                             __( 'POWr Order Form' ),
                             array( 'description' => __( 'Order Form by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-order-form' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Order Form.
        </p>
        <p>
    ...
    function register_powr_order_form() {
      register_widget( 'Powr_Order_Form' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_order_form' );
    //Create short codes for adding plugins anywhere:
    function powr_order_form_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-order-form' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-order-form' label='$label'></div>";
      }else{
      	"<div class='powr-order-form'></div>";
      }
    }
    add_shortcode( 'powr-order-form', 'powr_order_form_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_order_form_menu' );
    function powr_order_form_menu() {
      add_menu_page( 'POWr Order Form', 'POWr Order Form', 'manage_options', 'powr-order-form-settings', 'powr_order_form_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_order_form_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/order-form';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/order-form';
      }
    ...
    register_activation_hook(__FILE__, 'powr_order_form_plugin_activate');
      add_action('admin_init', 'powr_order_form_plugin_redirect');

      function powr_order_form_plugin_activate() {
      add_option('powr_order_form_plugin_do_activation_redirect', true);
      }
      ...
      function powr_order_form_plugin_redirect() {
      if (get_option('powr_order_form_plugin_do_activation_redirect', false)) {
          delete_option('powr_order_form_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-order-form-settings&' );
          }
       }
      }
